...
{
    private static final Button first = Button.secondary("first", Emoji.fromFormatted("<:first:930264043564961822>"));
    private static final Button previous = Button.secondary("previous", Emoji.fromFormatted("<:left:915425233215827968>"));
    private static final Button next = Button.secondary("next", Emoji.fromFormatted("<:right:915425310592356382>"));
    private static final Button last = Button.secondary("last", Emoji.fromFormatted("<:last:930264202331975701>"));
                    ...
                    switch (event.getComponentId())
                    {
                        ...
                        case "stop" -> {
                            ...
                            if (!event.getMessage().isEphemeral())
                                ...
                                event.editMessageEmbeds(getEmbed(page)).setActionRow(Collections.emptyList()).queue();
                        }
                        case "first" -> {
                            page = 1;
                            event.editMessageEmbeds(getEmbed(this.page)).setComponents(getButtonLayout(page)).queue();
