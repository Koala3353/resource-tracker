...
public class OnReadyEvent extends ListenerAdapter {
    ...
    public void onReady(@NotNull ReadyEvent event) {
        ...
        try (final PreparedStatement preparedStatement = SQLiteDataSource.getConnection()
                .prepareStatement("CREATE TABLE IF NOT EXISTS Resources (" +
                        "UserId INTEGER," +
                        items() + ")")) {
            LOGGER.info("Made a new table (Resources)");
    ...
    private String items() {
        ...
        while (x < names.size()) {
            Objects objects = Initializer.allItems.get(ResourcesDataUtils.filter(names.get(x)));
            items.append(ResourcesDataUtils.filter(objects.getName())).append(" INTEGER DEFAULT 0");
            if (x + 1 < names.size()) {
                items.append(", ");
            }
            x++;
